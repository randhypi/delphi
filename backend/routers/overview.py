    ...
    main_df = await db.run_query("""
        SELECT
            ...
            COALESCE(SUM(CAST(AMOUNT AS BIGINT))
                FILTER (WHERE TYPE IN ('WDL','TRF','PUR','ADV') AND RC = '00'), 0)     AS total_revenue,
            ROUND(
                COUNT(*) FILTER (WHERE RC = '00') * 100.0
                / NULLIF(COUNT(*) FILTER (WHERE TYPE IN ('WDL','TRF','PUR','ADV')), 0), 1
            )                                                                           AS success_rate,
            COALESCE(AVG(CAST(AMOUNT AS DOUBLE))
                FILTER (WHERE TYPE IN ('WDL','TRF','PUR','ADV') AND RC = '00'), 0)     AS avg_ticket,
